...
class BasicCatalogTest(unittest.TestCase):
    ...
    def test_basic_list_log(self):
        ...
        # do both
        skip3limit2 = \
        self.catalog.list_builds(self.cUtil.anonymous_ctx(), {'skip': 3, 'limit': 2})[0]
        ...
        complete_skip2limit1 = self.catalog.list_builds(self.cUtil.anonymous_ctx(),
                                                        {'only_complete': 1, 'skip': 2,
                                                         'limit': 1})[0]
        self.assertEqual(len(complete_skip2limit1), 1)
        self.assertEqual(complete[2], complete_skip2limit1[0])

        # get specific module => TODO: doesn't quite behave how you expect when passing multiple
        # modules with mixed git urls and module names
        module_builds = self.catalog.list_builds(self.cUtil.anonymous_ctx(),
                                                 {'modules': [{'module_name': 'onerepotest'},
                                                              {
                                                                  'module_name': 'registration_in_progress'}]
                                                  })[0]
